...
test("signed out: the account row is the sign-in home; managed connector still connects manually", async ({
  ...
  await expect(menu).toContainText("one-click connections need QunWork Cloud");
  ...
  await expect(modal.getByTestId("managed-connect")).toContainText("Sign in to QunWork Cloud");
