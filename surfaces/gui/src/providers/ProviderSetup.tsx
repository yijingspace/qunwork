...
export function relTime(epoch?: number | null, t?: (k: string, v?: Record<string, string | number>) => string): string | null {
  if (!epoch) return null;
  const secs = Math.max(0, Math.floor(Date.now() / 1000 - epoch));
  if (secs < 90) return t ? t("just now") : "just now";
  const mins = Math.floor(secs / 60);
  if (mins < 60) return t ? t("{n}m ago", { n: mins }) : `${mins}m ago`;
  const hrs = Math.floor(mins / 60);
  if (hrs < 48) return t ? t("{n}h ago", { n: hrs }) : `${hrs}h ago`;
  return t ? t("{n}d ago", { n: Math.floor(hrs / 24) }) : `${Math.floor(hrs / 24)}d ago`;
}
    ...
    if (p.configured && p.needs_key) {
      ...
      return (
        <span className="block text-[11.5px] text-ok font-medium truncate">
          ✓ {t("Connected")}{used ? <span className="text-muted font-normal"> · {t("used {n}", { n: used })}</span> : ""}
        </span>
      );
    }
    if (!p.needs_key)
    ...
    <div>
      ...
      {info?.needs_key && KEY_HELP[sel] && (
        <p className="text-[11.5px] text-faint mt-2">
          ...
          >
            {t("Create one at {url} ↗", { url: KEY_HELP[sel].label })}
          </button>{" "}
      ...
      {info && !info.needs_key && (
        <p className="text-[11.5px] text-faint mt-2">
          {t("No API key needed — Ollama runs models on this device.")}{" "}
          <button
            className="text-muted underline decoration-line underline-offset-2 hover:text-ink"
