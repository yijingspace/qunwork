    ...
    <Shell>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <PanelHead title={t("Automations")} sub={t("Recurring tasks QunWork runs on a schedule.")} />
        </div>
        <button
      ...
      <div className="text-[12px] text-faint flex gap-1.5 mb-4">
        ...
        <span>
          Runs only while qunwork-server is up — a missed schedule catches up once when it next
          starts.
        </span>
      </div>
      ...
      {empty ? (
        !showForm && (
          <div className={CARD + " p-4 text-[12.5px] text-muted"}>
            ...
            or just ask QunWork in a session.
          </div>
        )
      ) : (
      ...
      <div className="sched-detail">
        ...
        {editing ? (
          ...
          <div className="conn-meta">
            ...
            {task.enabled ? `Active · next ${fmt(task.next_run)}` : t("Paused")} · {task.schedule}
          </div>
        )}
