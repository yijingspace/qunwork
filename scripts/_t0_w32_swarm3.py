...
cur = con.cursor()
cur.execute("SELECT run_id, status, created_at, intent FROM orchestration_runs WHERE created_at >= '2026-08-03' AND created_at < '2026-08-10' ORDER BY created_at")
rows = cur.fetchall()
print("runs in 08-03..08-09:", len(rows))
print("status dist:", dict(Counter(r[1] for r in rows)))
for r in rows:
    print("  %s | %s | %s | %s" % (r[0], r[1], r[2], r[3][:70]))
